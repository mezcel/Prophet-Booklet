...
function loadPage(page) {
	...
	$.ajax({url: '/pages/page' + page + '.html'}).done(function(pageHtml) {
			$('.sj-book .p' + page).html(pageHtml.replace('Prophet Flipbook - Turnjs 4', ''));
		});
}
...
function addPage(page, book) {
	...
	if (!book.turn('hasPage', page)) {
		...
		if (book.turn('addPage', element, page)) {
			//loadPage(page);
			console.log("if (book.turn('addPage', element, page))", "true");
		}
...
/* in the Prophet Flipbook I did not make a Preview Image strip yet */
	...
	if (preview.css('background-image')==='' || preview.css('background-image')=='none') {
		...
		preview.css({content: '"' + view + '"'});
		preview.css({color: "green"});
		*/
